...
コードがうまく動かないときは、デバッグ（問題を探す）ことが大切です。

### console.log で値を確認する

`console.log()` を使うと、変数の値を確認できます。
...
function Counter() {
  ...
  const handleClick = () => {
    console.log('クリック前:', count);
    setCount(count + 1);
    console.log('クリック後:', count);  // 注意: まだ古い値が表示される
  };
...
ブラウザの開発者ツール（`F12`）の Console タブで、ログが確認できます。

### ブラウザの開発者ツール

開発者ツールは、デバッグに欠かせないツールです。
...
| タブ | 用途 |
|------|------|
| **Console** | エラーメッセージや `console.log` の出力を確認 |
| **Elements** | HTML の構造を確認・編集 |
| **Network** | ネットワーク通信を確認 |

### React DevTools

React 専用の開発者ツールです。コンポーネントの state や props を視覚的に確認できます。

**インストール方法:**

1. Chrome または Firefox の拡張機能ストアで「React Developer Tools」を検索
2. インストール
3. ブラウザを再起動

インストール後、開発者ツールに「Components」タブが追加されます。コンポーネントの構造や state の値が確認できます。
