...
GitHub は、インターネット上にコードを保存・共有するサービスです。Git で管理したコードを GitHub にアップロード（push）することで、以下のメリットがあります。
...
この本では Git の基本操作（init, add, commit）までを扱います。GitHub へのアップロード（push/pull）やリモートリポジトリの操作については、次のステップで学びましょう。

:::message
個人で練習する場合でも、Git の基本操作（init, add, commit）を覚えておくと、後で GitHub を使うときにスムーズに進められます。
:::
